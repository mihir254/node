...
class SpecReporter extends Transform {
  ...
  #handleTestReportEvent(type, data) {
    ...
    while (this.#stack.length) {
      ...
      prefix += `${this.#indent(msg.nesting)}${reporterUnicodeSymbolMap['arrow:right']}${msg.name}\n`;
    }
  ...
  #handleEvent({ type, data }) {
    switch (type) {
      ...
      case 'test:diagnostic':
        return `${this.#colors[type]}${this.#indent(data.nesting)}${reporterUnicodeSymbolMap[type]}${data.message}${colors.white}\n`;
      case 'test:coverage':
        return getCoverageReport(this.#indent(data.nesting), data.summary, reporterUnicodeSymbolMap['test:coverage'], colors.blue, true);
    }
  }
  ...
  _flush(callback) {
    ...
    const results = [`\n${this.#colors['test:fail']}${reporterUnicodeSymbolMap['test:fail']}failing tests:${colors.white}\n`];
